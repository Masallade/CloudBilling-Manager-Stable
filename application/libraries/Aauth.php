...
class Aauth
{
    ...
    public function __construct()
    {
        ...
        if (!isset($this->CI->_query)) {
            // Try to load database if not already loaded
            if (!isset($this->CI->db)) {
                $this->CI->load->database();
            }
            // Set _query if database is loaded (security feature)
            if (isset($this->CI->db) && !isset($this->CI->_query)) {
                $this->CI->_query = hash_hmac('sha1', $this->CI->config->base_url(), substr('i'.$this->CI->config->base_url(), 0, 20));
            }
        }
        ob_get_clean();
    }
    ...
    public function get_customer($user_id = FALSE)
    {


        $query = $this->aauth_db->where('id', $user_id);
        $query = $this->aauth_db->get('geopos_customers');
    ...
    public function premission($module_or_key)
    {
        // Backward-compatible: if a string key is passed, route to new permissions system
        if (!is_numeric($module_or_key)) {
            return $this->permissions_new(null, $module_or_key);
        }

        // Legacy numeric module id permission check
        ...
        $this->aauth_db->where('id', $module_or_key);
        $query = $this->aauth_db->get('geopos_premissions');
        $out = $query->row_array();
        
        // Check if result exists and has the role key
        if ($out === null || !is_array($out) || !isset($out[$role])) {
            return false;
        }
        
        return $out[$role];
    }
    public function permission_new($role_id = null, $permission_key = null)
    {
        ...
        if ($role_id === null) {
            $this->CI->db->reset_query(); // <- Add this line
        ...
        $this->CI->db->select('permission_value')
        ->from('role_permissions')
        ->where('role_id', $role_id)
        ->where('permission_key', $permission_key);
        $query = $this->CI->db->get();
        $result = $query->row();
        // var_dump('permission_new', 'role_id', $role_id, 'permission_key', $permission_key, 'result', $result); die();
        
        return $result && $result->permission_value == 1;
    }

    /**
     * Check multiple permissions for a role/user with AND/OR semantics
     * @param int|null $role_id Role ID to check, defaults to current user's role
     * @param array|string|null $permission_keys Array of keys, pipe-separated string for OR, ampersand-separated for AND, or single key
     * @param string $operator 'AND' or 'OR' when $permission_keys is an array (default 'AND')
     * @return bool
     */
    public function permissions_new($role_id = null, $permission_keys = null, $operator = 'AND')
    {
        // Resolve role id from current session if not provided
        $id = $this->CI->session->userdata('id');
        if (!$id) {
            return false;
        }
        ...
        // If no keys specified, allow (used as generic guard that user has a role)
        if ($permission_keys === null) {
            return true;
        }

        // Normalize input to array and operator
        if (is_string($permission_keys)) {
            if (strpos($permission_keys, '|') !== false) {
                $permission_keys = array_filter(array_map('trim', explode('|', $permission_keys)));
                $operator = 'OR';
            } else if (strpos($permission_keys, '&') !== false) {
                $permission_keys = array_filter(array_map('trim', explode('&', $permission_keys)));
                $operator = 'AND';
            } else {
                $permission_keys = [trim($permission_keys)];
            }
        } else if (is_array($permission_keys)) {
            $permission_keys = array_values(array_filter(array_map('trim', $permission_keys), function ($k) {
                return $k !== '';
            }));
        ...
        // Fetch all requested keys in a single query
        $this->CI->db->reset_query();
        $this->CI->db->select('permission_key, permission_value')
            ->from('role_permissions')
            ->where('role_id', $role_id)
            ->where_in('permission_key', $permission_keys);
        $query = $this->CI->db->get();
        $rows = $query->result();
        ...
        $granted = array();
        foreach ($rows as $row) {
            $granted[$row->permission_key] = ((string)$row->permission_value === '1');
        }

        if ($operator === 'OR') {
            foreach ($permission_keys as $key) {
                if (!empty($granted[$key])) {
                    return true;
                }
            }
            return false;
    ...
    public function delete_user($user_id)
    {
        ...
        // delete from user_to_group (only if table exists)
        if (isset($this->config_vars['user_to_group']) && $this->table_exists($this->config_vars['user_to_group'])) {
            $this->aauth_db->where('user_id', $user_id);
     ...
     * @return object|bool User object if found, FALSE if not found
     */
     ...
     * @param int $user_id User id to assign role to
     * @param int $role_id Role id to assign
     * @return bool Assignment success/failure
     */
    public function assign_user($user_id, $role_id)
    {
    ...
    private function table_exists($table_name)
    {
        ...
        $query = $this->aauth_db->query("SHOW TABLES LIKE '{$table_name}'");
