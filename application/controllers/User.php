...
class User extends CI_Controller
{
    ...
    public function index()
    {
        ...
        } else if ($this->input->get('e1')) {
            $data['response'] = ' The User is Already Logged in on Another Device!';
        }
    ...
    public function update_activity()
    {
        ...
        if ($this->aauth->is_loggedin()) {
            ...
            $this->db->where('id', $this->aauth->get_user()->id)
                ->update('geopos_users', ['last_activity' => date('Y-m-d H:i:s')]);
    ...
    //     $this->User_model->isUserLoggedIn($user);
    //     // Check if the user is already logged in using the model method
    //     if ($this->User_model->isUserLoggedIn($user)) {

    //         // User is already logged in
    //         // Handle the case, e.g., show an error message or redirect
    //         redirect('/user/?e1=eyxde', 'refresh'); // You may want to handle and display the error message
    //     } else {
    ...
    //             $this->User_model->markUserAsLoggedOut($user);

    //             $sessions_array = array();

    //             $query = $this->db->select('data, id') // Pass both columns as a single string
    //                 ->from('ci_sessions')
    //                 ->get();
    ...
    //                         $parts = explode('|', $pair);
    //                         if ($parts[0] == 'email') {
    //                             $email = trim(substr($parts[1], strpos($parts[1], ':') + 2), '"');
    //                             $email = preg_replace('/^\d+:\s*/', '', $email);
    //                             break;
    //                         }
    //                     }
    ...
    //                         array_push($sessions_array, $id);
    //                     }
    //                 }
    //             }

    //             if (!empty($sessions_array)) {
    //                 $this->db->where_in('id', $sessions_array);
    //                 $this->db->delete('ci_sessions');
    //             }
    //         }

    //         if ($this->aauth->login($user, $password, $rem, $this->captcha)) {
    //             // Update the is_logged_in column to 1 to indicate the user is logged in
    //             $this->User_model->markUserAsLoggedIn($user);
    ...
    public function deleteExpiredSessions()
    {
        ...
        $current_time = time();
        $sessionExpiration = $this->config->item('sess_expiration'); // Get session expiration in seconds

        $this->db->where("($current_time - login_time) >", $sessionExpiration);
    ...
    public function send_reset()
    {
        ...
        if ($out) {
            $this->load->model('communication_model');
            $mailtoc = $out['email'];
            $mailtotitle = $out['username'];
            $subject = '[' . $this->config->item('ctitle') . '] Password Reset Link';
            $link = base_url('user/reset_pass?code=' . $out['vcode'] . '&email=' . $email);
            $message = "<h4>Dear $mailtotitle</h4>, <p>Use the following link to reset your password:</p><p><a href='$link'>$link</a></p><p>Regards,<br>Team " . $this->config->item('ctitle') . "</p>";

            $this->communication_model->send_email($mailtoc, $mailtotitle, $subject, $message);
            echo json_encode(['status' => 'Success', 'message' => 'Email Sent Successfully!']);
        } else {
            echo json_encode(['status' => 'Error', 'message' => 'Failed to process password reset.']);
        }
    }
    ...
    //         $this->User_model->markUserAsLoggedOut($user->email);
    //         $this->aauth->logout();
    //     }
    //     redirect('/user/', 'refresh');
    // }
