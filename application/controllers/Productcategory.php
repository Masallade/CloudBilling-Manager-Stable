...
class Productcategory extends CI_Controller
{
    ...
    {
        if (!$this->aauth->permission_new(null, 'stockWarehouses')) {
    ...
    {
        if ($this->aauth->permission_new(null, 'stockDeleteCategory')) {
            $id = intval($this->input->post('deleteid'));
            if ($id) {
                // Check if category has linked products
                $this->db->where('pcat', $id);
                $product_count = $this->db->count_all_results('geopos_products');
                
                if ($product_count > 0) {
                    // Category has linked products - prevent deletion
                    echo json_encode(array(
                        'status' => 'Error', 
                        'message' => 'Cannot delete category. This category is linked to ' . $product_count . ' product(s). Please remove or reassign all products from this category before deleting.'
                    ));
                    return;
                }
                
                // No products linked - safe to delete
                // Delete any related movers records first
                $query = $this->db->query("DELETE geopos_movers FROM geopos_movers LEFT JOIN geopos_product_cat ON  geopos_movers.rid1=geopos_product_cat.id WHERE geopos_product_cat.id='$id' AND  geopos_movers.d_type='1'");
                
                // Delete the category
                $this->db->delete('geopos_product_cat', array('id' => $id));
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('Product Category deleted successfully')));
            } else {
    ...
    {
        if ($this->aauth->permission_new(null, 'stockWarehouseDelete')) {
            $id = intval($this->input->post('deleteid'));
            if ($id) {
                // Check if warehouse has linked products
                $this->db->where('warehouse', $id);
                $product_count = $this->db->count_all_results('geopos_products');
                
                if ($product_count > 0) {
                    // Warehouse has linked products - prevent deletion
                    echo json_encode(array(
                        'status' => 'Error', 
                        'message' => 'Cannot delete warehouse. This warehouse is linked to ' . $product_count . ' product(s). Please remove or reassign all products from this warehouse before deleting.'
                    ));
                    return;
                }
                ...
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('Product Warehouse deleted successfully')));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
            }
        } else {
...
//view for edit
    ...
    public function report_product()
    {
        ...
        $lang_to_load = isset($system_lang['lang']) && !empty($system_lang['lang']) ? $system_lang['lang'] : 'english';
        ...
        if ($pid && $r_type) {
            ...
            $html = $this->load->view('products/cat_statementpdf-ltr', array('report' => $result, 'product' => $product, 'r_type' => $r_type, 'company' => $company), true);
