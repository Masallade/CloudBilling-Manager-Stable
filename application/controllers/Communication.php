...
class Communication extends CI_Controller
{
    ...
    {
        if (!$this->aauth->permission_new(null, 'suppliersSendMessage')) {
            echo json_encode(array("status" => "Error", "message" => "You do not have permission to send invoices."));
            return;
        }
        ...
        $result = $this->communication_model->send_email($mailtoc, $mailtotilte, $subject, $message, $attachmenttrue, $attachment);
    ...
    {
        if (!$this->aauth->permission_new(null, 'suppliersSendMessage')) {
            echo json_encode(array("status" => "Error", "message" => "You do not have permission to send messages."));
            return;
        }
