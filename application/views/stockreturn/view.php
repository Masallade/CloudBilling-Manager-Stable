                                ...
                                <tbody>
                                    <?php $c = 1;
                                        ...
                                        foreach ($products as $row) {
                                            ...
                                            <td>' . amountExchange($row['qty'] * $rate, 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($rate) . ')</td>
...
<th scope="row">' . $c . '</th>
                            ...
                            <td>' . amountExchange($row['price'], 0, $this->aauth->get_user()->loc) . '</td>
                             <td>' . amountFormat_general($row['qty']) . $row['unit'] . '</td>
                              <td>' . amountExchange($row['totaldiscount'], 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($row['discount']) . $this->lang->line($invoice['format_discount']) . ')</td>
                            <td>' . amountExchange($row['tax'] * $row['qty'], 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($row['tax']) . ')</td>
                                    ...
                                    <?php $c = 1;
                                        ...
                                        foreach ($products as $row) {
                                            ...
                                            echo '<tr>
                                                    ...
                                                    <td>' . amountExchange($row['tax'] * $row['qty'], 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($row['tax']) . ')</td>
                                                    <td>' . amountExchange($row['totaldiscount'], 0, $this->aauth->get_user()->loc) . ' (' . amountFormat_s($row['discount']) . $this->lang->line($invoice['format_discount']) . ')</td>
                                                    <td>' . amountExchange($row['qty'] * $row['price'], 0, $this->aauth->get_user()->loc) . '</td>
                                                </tr>';
                                            echo '<tr><td colspan=5>' . $row['product_des'] . '</td></tr>';
                                            $c++;
                                        } ?>
