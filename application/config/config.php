...
|
*/
$config['base_url'] = 'http://192.168.100.83/demo.cloudbillingmanager.com/';
...
// Function to get tax_type from database using direct connection (before CI is initialized)
function getTaxTypeFromDB() {
    // Default tax type
    $tax_type = 'VAT';
    ...
    if (file_exists($db_config_file)) {
        ...
        if (isset($db['default'])) {
            ...
            if (!$mysqli->connect_error) {
                // Query to get tax_type from geopos_system table (single row, no id needed)
                $query = "SELECT tax_type FROM geopos_system LIMIT 1";
