...

{
    ...
    function supplier_ammount($custid)
    {
        $this->db->select('SUM(total) AS total, SUM(pamnt) AS pamnt, (SUM(total) - SUM(pamnt)) AS expense');
        ...
        // dd($result);
        // Now you can use $result to access the values
    ...
    public function add($name, $company, $phone, $phone1, $email, $email2, $email3, $address, $city, $region, $country, $postbox, $taxid)
    ...
    public function edit($id, $name, $company, $phone, $phone1, $email, $email2, $email3, $address, $city, $region, $country, $postbox, $taxid)

    {
        ...
        // Return boolean result instead of echoing JSON (controller will handle response)
        return $this->db->update('geopos_supplier');
    }
    ...
    public function sales_due($sdate, $edate, $csd, $trans_type, $pay = true, $amount = 0, $acc = 0, $pay_method = '', $note = '')
    {
        if ($pay) {
            ...
            if ($amount) {
                ...
                $note = ''; // Initialize the note string
                $processed_ids = []; // Track processed purchase IDs
                ...
                foreach ($result as $row) {
                    ...
                    if ($amount_custom >= $due) {
                        ...
                        $amount_custom -= $due;
                    } elseif ($amount_custom > 0 && $amount_custom < $due) {
                ...
                // Fetch the inv_id and invoice_ref for the processed IDs
                $this->db->select('inv_id, invoice_ref');
                $this->db->from('geopos_purchase');
                $this->db->where_in('id', $processed_ids);
                $query = $this->db->get();
                $inv_ids = $query->result_array();
                ...
                $data = array(
                    ...
                    'tid' => 0,
                    'ext' => 1,
                    'note' => 'Bulk Payment For ' . $note,
                    'loc' => $this->aauth->get_user()->loc,
                    'trans_ref' => $inv_ids[0]['inv_id'], // Set only one value
                    'inv_id' => $inv_ids[0]['invoice_ref'] // Set only one value
                );
