...
class Products_model extends CI_Model
{
    ...
    var $column_order = array(null, 'geopos_products.product_name', 'geopos_products.qty', 'geopos_products.product_code', 'geopos_product_cat.title', 'geopos_products.product_price', 'geopos_products.fproduct_price');
    var $column_search = array('geopos_products.product_name', 'geopos_products.product_code', 'geopos_product_cat.title');
    ...
    {
        $this->db->select('geopos_products.pid,geopos_products.product_name,geopos_products.qty,geopos_products.product_code,geopos_product_cat.title AS c_title,geopos_products.product_price,geopos_products.fproduct_price');
        ...
        // Global search (limited fields for performance)
        $search = $this->input->post('search');
        $value = isset($search['value']) ? $search['value'] : '';
        if ($value !== '') {
            $this->db->group_start();
            $this->db->like('geopos_products.product_name', $value);
            $this->db->or_like('geopos_products.product_code', $value);
            $this->db->or_like('geopos_product_cat.title', $value);
            $this->db->group_end();
        }
    ...
    private function _get_datatables_query_filtered($id = '', $w = '', $sub = '')
    {
        $this->db->select('geopos_products.pid,geopos_products.product_name,geopos_products.qty,geopos_products.product_code,geopos_product_cat.title AS c_title,geopos_products.product_price,geopos_products.fproduct_price');
        $this->db->from($this->table);
        $this->db->join('geopos_warehouse', 'geopos_warehouse.id = geopos_products.warehouse');
        ...
        if ($_POST['filter'] == 2) {
            $this->db->where('geopos_products.qty  <=', 0);
        }
        if ($_POST['filter'] == 1) {
            $this->db->where('geopos_products.qty  >', 0);
        }
    ...
    // private function _get_datatables_query_filtered()
    //     {
    ...
    //         if($_POST['filter']==1)
    //         {
    //              $sql='SELECT geopos_products.pid,geopos_products.product_name, geopos_products.qty, geopos_products.product_code,geopos_product_cat.title as category,geopos_products.fproduct_price as price FROM geopos_products JOIN geopos_product_cat ON geopos_products.pcat= geopos_product_cat.id WHERE geopos_products.qty>0 limit ' .$start.','.$end.'';
    //         }
    //         else{
    //              $sql='SELECT geopos_products.pid,geopos_products.product_name, geopos_products.qty, geopos_products.product_code,geopos_product_cat.title as category,geopos_products.fproduct_price as price FROM geopos_products JOIN geopos_product_cat ON geopos_products.pcat= geopos_product_cat.id WHERE geopos_products.qty<=0 limit ' .$start.','.$end.'';
    //         }
    ...
    // function get_datatables_filtered()
    // {

    //     $data=$this->_get_datatables_query_filtered();
    ...
    function count_filtered_filtered($id, $w = '', $sub = '')
    {
        if ($id > 0) {
            $this->_get_datatables_query_filtered($id, $w, $sub);
        } else {
            $this->_get_datatables_query_filtered();
        }
        $query = $this->db->get();
    ...
    function get_datatables_filtered($id = '', $w = '', $sub = '')
    {
        if ($id > 0) {
            $this->_get_datatables_query_filtered($id, $w, $sub);
        } else {
    ...
    public function addnew(
        ...
        // Validate warehouse - it cannot be null or empty
        if (empty($warehouse) || $warehouse == '' || $warehouse == null) {
            echo json_encode(array('status' => 'Error', 'message' => 'Warehouse is required. Please select a warehouse first before creating a product.'));
            return;
        }
        
        $ware_valid = $this->valid_warehouse($warehouse);
        
        // Additional check if warehouse validation fails
        if (!$ware_valid || empty($ware_valid) || !isset($ware_valid['id'])) {
            echo json_encode(array('status' => 'Error', 'message' => 'Invalid warehouse selected. Please select a valid warehouse.'));
            return;
        }
        ...
        if ($this->aauth->get_user()->loc) {
            if (isset($ware_valid['loc']) && ($ware_valid['loc'] == $this->aauth->get_user()->loc or $ware_valid['loc'] == '0' or $warehouse == 0)) {
    ...
    public function edit($p_to_b, $pid, $catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $code_type, $sub_cat = '', $b_id = '', $vari = null, $serial = null, $weight_unit, $weight_qty, $wdate = null)
    {
        $this->db->select('qty');
        ...
        // Process expiry date
        $expiry_date = null;
        if ($wdate && !empty($wdate) && $wdate != '0000-00-00' && $wdate != '0000-00-00 00:00:00') {
            $datetime1 = new DateTime(date('Y-m-d'));
            $datetime2 = new DateTime($wdate);
            $difference = $datetime1->diff($datetime2);
            if ($difference->d > 0 || $datetime2 > $datetime1) {
                $expiry_date = $wdate;
            }
        }
    ...
    public function products_list_for_dropdown()
    {
        // Use caching for products list
        $cache_key = 'products_dropdown_' . $this->aauth->get_user()->loc;
        $this->load->driver('cache');
        
        $products = $this->cache->get($cache_key);
        
        if ($products === FALSE) {
            $this->db->select('pid, product_name, product_des, product_code, product_price, fproduct_price, taxrate, qty, unit, barcode, image, code_type');
            $this->db->from('geopos_products');
            $this->db->join('geopos_warehouse', 'geopos_warehouse.id = geopos_products.warehouse');
            ...
            $this->db->limit(100); // Limit to 100 products for performance
            $query = $this->db->get();
