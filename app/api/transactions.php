...
if($jwt){
  ...
  //print_r($c_id);die();
 $sql="select credit,debit,id from geopos_transactions where payerid=$c_id order by id asc";
            ...
            if ($result = mysqli_query($link, $sql)) {
                    if (mysqli_num_rows($result)){
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            $balance_array[$i]['balance']=$balance;
                            $balance_array[$i]['id']=$row['id'];
                            $i++;
                        }
                    }
            }
               ...
               $end_date=$_GET['end_date'];
                  $sql = "SELECT * FROM `geopos_transactions` WHERE `payerid` = ' $c_id' AND (DATE(date) BETWEEN '$start_date' AND '$end_date') AND `ext` =0 order by id DESC";
                if ($result = mysqli_query($link, $sql)) {
                    if (mysqli_num_rows($result)){
                        $i=0;
                        while ($row = mysqli_fetch_array($result)) {
                        ...
                        $transactions[$i]['balance']= number_format($balance_array[array_search($row[0], array_column($balance_array, 'id'))]['balance'], 2, '.', '');
                       ...
                       if($transactions[$i]['method']== 'Balance'){
				            $transactions[$i]['note'] = 'Payment trans-'.$row[0];
			            }
                        $i++;
                        }
                    }
