...
require_once "./include/configdb.php";
 ...
 $tomorrow = date("Y-m-d",strtotime("+1 day")); 
            ...
            $result = mysqli_query($link, $sql);
                ...
                while ($row = mysqli_fetch_array($result)) {
                 $chart_data[$i]['id']=$row[0];
                 $chart_data[$i]['total']=$row[1];
                 $chart_data[$i]['date']=$row[2];
                 $i++;
               }   
                              
            
            $sql="SELECT account,debit,credit,paymt_method FROM geopos_transactions inner join geopos_invoices on geopos_transactions.tid=geopos_invoices.id
            ...
            $sql="select id, tax, total from geopos_invoices where invoicedate ='".$today."'";
                  ...
                  $response = mysqli_query($link, $sql); 
                    $todayinvoices = mysqli_fetch_array($response);
                    var_dump($todayinvoices);exit();
         $invoiceids=array();
        ...
        if(empty($invoiceids)){
         ...
         $sql="select pid,qty,price,totaltax,subtotal from geopos_invoice_items where tid IN  (".$invoiceids.")";
                ...
                $response = mysqli_query($link, $sql); 
                    $ivoice_items = mysqli_fetch_array($response);
 ...
 $total= $invoice_items_total-$products_price_total;
